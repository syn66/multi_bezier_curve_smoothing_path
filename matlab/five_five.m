...
%第二条五阶贝塞尔曲线
second_5_bezier_x = r0(1).*(1-t1).^5 + 5.*r1(1).*t1.*(1-t1).^4 + 10.*r2(1).*t1.^2.*(1-t1).^3 + 10.*r3(1).*t1.^3.*(1-t1).^2 + 5.*r4(1).*t1.^4.*(1-t1) + r5(1).*t1.^5;
second_5_bezier_y = r0(2).*(1-t1).^5 + 5.*r1(2).*t1.*(1-t1).^4 + 10.*r2(2).*t1.^2.*(1-t1).^3 + 10.*r3(2).*t1.^3.*(1-t1).^2 + 5.*r4(2).*t1.^4.*(1-t1) + r5(2).*t1.^5;
...
dot_x_2 = 5*(r1(1) - r0(1)).*(1-t).^4 + 20.*(r2(1) - r1(1)).*t.*(1-t).^3 + 30*(r3(1) - r2(1)).*t.*t.*(1-t).*(1-t)  + 20.*(r4(1) - r3(1)).*t.*t.*t.*(1-t) + 5*(r5(1) - r4(1)).*t.^4;
dot_y_2 = 5*(r1(2) - r0(2)).*(1-t).^4 + 20.*(r2(2) - r1(2)).*t.*(1-t).^3 + 30*(r3(2) - r2(2)).*t.*t.*(1-t).*(1-t)  + 20.*(r4(2) - r3(2)).*t.*t.*t.*(1-t) + 5*(r5(2) - r4(2)).*t.^4;

dot_dot_x_2 = 20*(r2(1) - 2*r1(1) + r0(1)).*(1-t).^3 + 60*(r3(1) - 2*r2(1) + r1(1)).*t.*(1-t).*(1-t) + 60.*(r4(1) - 2.*r3(1) + r2(1)).*t.*t.*(1-t) + 20.*(r5(1)-2*r4(1) + r3(1)).*(t).^3; 
dot_dot_y_2 = 20*(r2(2) - 2*r1(2) + r0(2)).*(1-t).^3 + 60*(r3(2) - 2*r2(2) + r1(2)).*t.*(1-t).*(1-t) + 60.*(r4(2) - 2.*r3(2) + r2(2)).*t.*t.*(1-t) + 20.*(r5(2)-2*r4(2) + r3(2)).*(t).^3;

curvature2 = (dot_x_2.*dot_dot_y_2 - dot_dot_x_2.*dot_y_2) ./ (dot_y_2.*dot_y_2 + dot_x_2.*dot_x_2).^1.5;

curvature = [curvature1 curvature2];
x = [first_5_bezier_x second_5_bezier_x];
...
xlabel("X(m)")
ylabel("Y(m)")
plot(second_5_bezier_x, second_5_bezier_y,"b-","LineWidth",2,"DisplayName","第二段五阶贝塞尔曲线")
plot([r0(1),r1(1),r2(1),r3(1),r4(1),r5(1)],[r0(2),r1(2),r2(2),r3(2),r4(2),r5(2)],'g.','MarkerSize',30,"DisplayName","第二段五阶贝塞尔曲线的控制点")

plot(r1(1) , r1(2),'.','MarkerSize',30,"DisplayName","第二段五阶贝塞尔曲线的辅助控制点" )
